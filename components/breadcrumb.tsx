...
export default function Breadcrumb() {
  ...
  return (
    <nav className="py-4 bg-[#0066a4]/5" aria-label="Breadcrumb">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <ol className="flex items-center space-x-2 text-gray-600 text-sm md:text-base">
          {breadcrumbs.map((item, index) => (
            ...
            >
              {index > 0 && <ChevronRight className="h-4 w-4 text-gray-400 mx-2 flex-shrink-0" aria-hidden="true" />}
              {index === breadcrumbs.length - 1 ? (
                ...
                >
                  {item.label === "Home" ? <Home className="h-4 w-4 mr-1" /> : item.label}
                </Link>
              )}
            </motion.li>
          ))}
